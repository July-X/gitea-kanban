...
describe('TimelineNewView allRows + relation 占位（dot 与 commit-row 行节奏对齐）', () => {
  ...
  it('dot 与 commit-row 行节奏对齐：commit-row 固定高度 24px', () => {
    // v1.6 回归固定高度：分支名完整显示 + 提交信息省略号 → 行高不变 → dot 永远对齐
    const rowMatch = viewSource.match(/\.commit-row\s*\{([^}]+)\}/);
    expect(rowMatch).not.toBeNull();
    const block = rowMatch![1]!;
    const hasCssHeight = block.includes('height: 24px');
    const hasInlineHeight = /height:\s*ROW_H/.test(viewSource);
    expect(hasCssHeight || hasInlineHeight).toBe(true);
    // 必须 nowrap 保证单行
    expect(block).toMatch(/white-space:\s*nowrap/);
    // relation 行保持固定 24px（无内容不需要弹性）
    const relMatch = viewSource.match(/\.commit-row--relation\s*\{([^}]+)\}/);
    expect(relMatch).not.toBeNull();
    expect(relMatch![1]!).toMatch(/height:\s*24px/);
  });
});
