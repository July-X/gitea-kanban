...
>
> **v0.8.33 增量**：回退 v0.8.25.7 的「lane 左移压缩」——实测 DeepSeek-Reasonix 301 commits 与 vscode-gitlens 截图对比，发现长分支链被挤到左侧少数 lane、右侧 lane 几乎全空，与 GitLens `commit-graph/src/engine/layout.ts:493-510` 原版算法不符（GitLens 真实行为：lane 释放后**只**从 `columnsUsed` 移除空 column，**不动**现有 reservation/segment 的 column，后续 commit 走 `claimNextColumn()` 复用最低空 column）。`app/git/graph/layout_gitlens.go` 删 `gitlensLayoutState.compact` 字段 + `compactLanes()` 函数体改 no-op（保留引用作历史 reference）+ `assignColumnForRow` 删 `if s.compact { ... }` 块 + `newGitlensLayoutState` / `gitlensAssignColumns` 删 `compact bool` 参数 + `BuildGraphGitlens` 删 v0.8.25.7 引入的"位置用压缩 lane + 颜色用未压缩逻辑 lane"双轨（删 `colorCols` 第二跑 + 删 `colorColOf` 映射 + 节点/边/merge 合入线 `Color` 全部从 `colorColOf[...] % 16` 改回 `colOf[...] % 16`）。测试同步：`TestBuildGraphGitlens_LaneCompactMatchesGitlens` 改名 `MatchesGitLensLaneReuse`（拓扑不变，预期 L 链稳定 lane 2 / X1 复用 lane 1 / 颜色=位置 / L 链 row 4-9 区域不跨 lane）+ `TestBuildGraphGitlens_ColorIsColumnMod16` 注释更新 + 6 处 `gitlensAssignColumns` 调用去 `compact` 参数。`app/git/graph/debug_dump_test.go` 加 `GRAPH_DEBUG_MAX_COUNT` 环境变量支持（默认 40，DeepSeek-Reasonix 截图用 301）。DeepSeek-Reasonix 301 commits lane 分布对比：lane 1 115→128（释放 lane 被后续 commit 复用）/ lane 7 26→2（长分支链不再被压到 lane 7）/ lane 9 4→28（长分支链回到原 lane）；maxLane 14→14 巧合（pinned head + 长分支链右端点不变）。`docs/design/gitgraph-engine.md` §2.2 改"双跑"为"单跑" + §5.1 改 Color 取色逻辑注释；`docs/design/gitgraph-engine-history.md` 新增"阶段 11"段记录踩坑教训（**不要"凭印象"实现算法** / **小仓库验证不能外推到大仓库** / **肉眼对比不能量化** / **不要为"消除双跑"以外的任何目的引入双跑**）。docs/releases/v0.8.33.md + docs/releases/INDEX.md 已同步。
>
> **v0.8.33.1 增量**：删除 `EnsureForkParentRemote`——用户 mid-turn 反馈 `50e771376ce4cddaa08e2c9e1370624327b4ffd3` 混入 git graph，调查发现是 `app/platform/github/adapter.go:EnsureForkParentRemote` 在 `CloneRepo` 完成后 + `app_sync.go:PullRepoByProjectId` 每次刷新时自动触发：调 `parentCloneURL` 查 fork parent clone URL → `EnsureRemote` 加 `org` remote → `FetchWithFilter` 拉 fork parent 全部 `refs/heads/*`。问题：用户 fork `esengine/DeepSeek-Reasonix`（活跃开源项目有 23 个 dependabot PR branch）后，应用数据目录仓库比源码仓库多 50 branches，上游 bot PR tip commit 50e77137 混进 git graph（maxLane 14→15）。`adapter.go:297 CloneRepo` + `app_sync.go:99 PullRepoByProjectId` 两处调用删除；`EnsureForkParentRemote` 函数体整体移除（行为错误：拉所有 refs/heads/* 含 dependabot PR，**与设计意图"拉 fork parent 装饰信息"脱节**——`parentCloneURL` 返回值在前端 0 消费方）；`parentCloneURL` 保留（无副作用的纯网络 API 调用，供未来 fork 关系 UI 用）；孤儿变量 `apiHostURL` 清理。**新原则**：只同步当前账号仓库基本信息，不关心 fork 源信息。`docs/releases/v0.8.33.1.md` + `docs/releases/INDEX.md` 已同步。
>
> **v0.8.34 增量**：修复 Git Graph「已到全部提交记录的末尾」永远误显示——用户截图显示应用 graph 1126 reachable commit 仓库只显示 301 条（maxCommits=300 截断），底部永远「已到全部」看不见 row 300 之后的 826 条更老 commit，root commit `327ac81` 不可见。根因：`app/git/graph/layout_gitlens.go:862` 硬编码 `Truncated: false` 导致 `LogCommits` 算的 `truncated = (len(commits) > opts.MaxCount)` 截断标志在 graph 层丢失，前端 `TimelineNewView.vue:967 allLoaded = !dto?.truncated` 永远 true → 哨兵 v-if=`!allLoaded` 永远隐藏 → 用户无法触发 `loadMoreGraph`。**修法**：`BuildGraphGitlens(commits, head, pinnedHeadShas, truncated bool)` 签名加 `truncated` 参数（替换硬编码 false）；`gitea/github adapter` `LogGraph` 调它时传 `logResult.Truncated`；`debug_dump_test.go` 同步用 `logResult.Truncated`；6 个测试文件调用点更新签名。新增 `TestBuildGraphGitlens_TruncatedPassesThrough` 回归测试。**对齐 vscode-gitlens**：`packages/git-cli/src/providers/graph.ts` `hasMore = (limit && count >= limit)` 语义完全等价——limit 触顶 → hasMore=true → `renderLoadMoreRow`（vscode-gitlens 增量 cursor-based paging）/ `loadMoreGraph` sentinel 可见（我们 window-based paging）。验证：MaxCount=300 → truncated=true；MaxCount=2000 → truncated=false。12 个 Go 包测试全 PASS + frontend vitest 25 gitgraph 测试 PASS + pnpm typecheck OK。`docs/releases/v0.8.34.md` + `docs/releases/INDEX.md` 已同步。
>
> **v0.8.35 增量**：ref badge 重做对齐 vscode-gitlens + 用户参考图——`.ref-badge` 高度 22 → 37（+68%），颜色 `#E6EDF3` 白色（lane 透明 bg + lane 色 border + hover 填充 lane 色 + 文字变深，对齐 vscode-gitlens `.gl-graph__ref-pill`）。`.ref-badge__icon` 18 → 30x30（+67%），margin-right 6 → 0（icon 紧贴 label 0 gap），stroke `--color-shell-main-bg` 写死 → `currentColor`（hover 时跟 badge 文字色变），Tag/GitBranch SVG size 14 → 24（1.2rem，对齐 vscode-gitlens `--code-icon-size: 1.2rem`）。**`VSCODE_GRID_Y` 24 → 50**（让 37px badge 居中 row + 上下 padding ~13px，对齐用户参考图 22）。`vscode-render.test.ts` 9 处 hardcode path 数值（控制点 `12 + gridY * 0.8`、贝塞尔端点 `offsetY + gridY`、angular 拐点 `(offsetY + gridY) - gridY * 0.38`、fork stitch 转场段等）改 VSCODE_GRID_Y 动态计算 + `.toFixed(1)` 对齐 SVG path toFixed 精度。25 个 gitgraph 测试全 PASS + pnpm typecheck OK。`docs/releases/v0.8.35.md` + `docs/releases/INDEX.md` 已同步。
>
