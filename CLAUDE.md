...
`gitea-kanban` 是一个基于 Gitea/GitHub 的桌面端看板 + Git Graph 工具，技术栈固定为 **Go + Wails v2 + Vue 3**（v1 时代的 Electron+TypeScript+SQLite 已迁移完成，旧代码归档到 `legacy/electron/`）。
...
- `legacy/electron/**`：v1 旧代码归档，**不再构建**，仅供参考
...
- Workspace 路径：默认 `~/.gitea-kanban/workspace`，repos 存 `${workspace}/repos/${owner}__${repo}/`
