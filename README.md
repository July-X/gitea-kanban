...
- **支持平台**：Gitea（完整）+ GitHub（列仓库 + Git Graph；议题/合并/标签/成员暂未支持）
...
|---|---|---|
| 验证 token | ✅ | ✅ |
| 列仓库 | ✅ | ✅（登录用户能访问的，含 collaborator / 组织成员） |
| 选仓库 + 同步 + Git Graph | ✅ | ✅（Go Git Graph：commit DAG + 分支 + 标签） |
| 议题 / 合并请求 / 标签 / 成员 | ✅ | ❌（首期未支持，UI 隐藏 + 后端返 `ErrNotSupported`） |
