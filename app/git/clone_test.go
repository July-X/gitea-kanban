...
func TestCloneRepo_NoCheckout_NoWorktreeFiles(t *testing.T) {
	...
	// v0.8.39.1: NoCheckout 模式 worktree 空 + index 空 → git status 全报 "D  filename"
	// detectUncommittedChanges 检测到 allDeleted=true → 不插入 UNCOMMITTED 虚拟 commit。
	// 所以 NoCheckout clone 不会有 UNCOMMITTED，只有真实 commit。
	if len(logResult.Commits) < 1 {
		t.Fatalf("expected at least 1 commit, got 0")
	}
	// commits[0] 应是 initial commit（不是 UNCOMMITTED）
	if logResult.Commits[0].Subject != "initial commit" {
		t.Errorf("commit[0] subject = %q, want %q", logResult.Commits[0].Subject, "initial commit")
	}
}
...
func TestCloneRepo_AllBranchesSynced(t *testing.T) {
	...
	// v0.8.39.1: NoCheckout 模式不插入 UNCOMMITTED（allDeleted=true 跳过）
	// 所以只有 2 个真实 commit (main commit + feature commit)
...
func TestCloneRepo_LargeRepoMode_ShallowSingleBranchNoTags(t *testing.T) {
	...
	// v0.8.39.1: NoCheckout 模式不插入 UNCOMMITTED（allDeleted=true 跳过）
	if len(logResult.Commits) != 1 {
		t.Fatalf("expected shallow clone to expose 1 commit (main), got %d", len(logResult.Commits))
	}
	if logResult.Commits[0].Subject != "main commit" {
		t.Fatalf("commits[0] 应是 main commit, 实际 %q", logResult.Commits[0].Subject)
	}
