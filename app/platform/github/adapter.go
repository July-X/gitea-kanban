...
func (a *GitHubAdapter) CloneRepo(ctx context.Context, hostURL, username, token, owner, repo, workspacePath, accountUsername string, progress git.ProgressCallback) (string, error) {
	...
	// v0.8.33+：删除 EnsureForkParentRemote 调用。原行为是 CloneRepo 完成后
	// 查 GitHub API 拿 fork parent clone URL → 加 'org' remote → FetchWithFilter
	// 拉 fork parent 全部 refs/heads/*。问题：上游 esengine/DeepSeek-Reasonix
	// 这类活跃开源项目有 23 个 dependabot PR branch，会全部被拉进用户的
	// fork 仓库本地引用，让 50e77137 这类上游 bot commit 混进 git graph。
	// 新原则：只同步当前账号仓库基本信息，不关心 fork 源信息。
	_ = a
...
func (a *GitHubAdapter) LogGraph(ctx context.Context, localPath string, opts platform.LogGraphOpts) (*platform.GraphResult, error) {
	...
	// v0.8.26：layout 算法统一收敛到 GitLens GKC；v0.8.25 env 切换开关已删除。
