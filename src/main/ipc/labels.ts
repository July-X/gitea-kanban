...
import { listGiteaLabels, createGiteaLabel } from '../gitea/labels.js';
...
async function listLabelsHandler(args: ListLabelsArgs): Promise<ListLabelsResp> {
  ...
  logger.info(
    { op: 'labels.list', latencyMs: Date.now() - start, count: result.items.length },
    'ipc done',
  );
...
async function createLabelHandler(args: CreateLabelArgs): Promise<LabelDto> {
  const start = Date.now();
  logger.info(
    { op: 'labels.create', args: { projectId: args.projectId, name: args.name } },
    'ipc start',
  );
  const proj = resolveProject(args.projectId);
  ...
  logger.info(
    { op: 'labels.create', latencyMs: Date.now() - start, labelId: result.id },
    'ipc done',
  );
  return result;
}
