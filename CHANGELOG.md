...
## v1.3.1 — 撤回看板键盘双模（聚焦鼠标主路径）(2026-06-15)

**触发**：v1.3 收口后用户拍板——"键盘双模"是实施过程中的自行扩张，不该引入；非技术受众（PM / 设计师 / 市场 / 运营）几乎全部走鼠标路径，键盘双模的测试成本与 UX 价值不匹配。

**撤回范围**（v1.3 commit `635dc1c` 里同步引入的"键盘拖拽"那一半）：

| 文件 | 撤回内容 |
|---|---|
| `src/renderer/composables/useKanbanKeyboardDrag.ts` | 整个删除（132 行：状态机 + onCardKeydown + window keydown Esc 兜底） |
| `src/renderer/composables/__tests__/useKanbanKeyboardDrag.test.ts` | 整个删除（10 用例） |
| `src/renderer/lib/drag-helper.ts` | 剥 `keyDownToColumn` / `KeyboardDragState` / `makeIdleKeyboardDrag` |
| `src/renderer/lib/__tests__/drag-helper.test.ts` | 删 2 个 describe（13 用例 keyDownToColumn + 1 用例 makeIdleKeyboardDrag = 14 用例） |
| `src/renderer/components/board/KanbanColumnSection.vue` | 剥 `keyboardPickedIssueId` / `keyboardHoveredColumnId` props + `card--picked` / `column__cards--drag-target` class + `card-keydown` emit + `@keydown` + `CardKeydownPayload` |
| `src/renderer/views/BoardView.vue` | 删 `useKanbanKeyboardDrag` 导入 + 调用 + `onCardKeydown` 包装 + `keyboardPickedIssueId` computed + `board__drag-banner` 模板块 + 顺手清掉没人用的 `showToast` / `ColumnDto` / `IssueCardDto` import |
| `tests/e2e/board-drag.spec.ts` | 删"用例 3：键盘 Space 拾起 + 方向键 + Space 放下"（1 用例）+ `keyDownToColumn` / `makeIdleKeyboardDrag` import + 顶部说明改写 |

**保留**：vue-draggable-plus 鼠标拖拽、Tab 聚焦时的卡片 actions 显隐（`.card:focus-within` CSS）、`tabindex="0"` / `role="article"` / `aria-label`（驱动屏幕阅读器语义）。`isFinishColumnByTitle` 留在 `drag-helper.ts`（鼠标拖到"已完成"列仍触发二次确认）。

**用户价值判断**：
- 键盘双模 UX 边缘价值——非技术受众记忆 Space / 方向键 / Esc 组合的意愿极低
- 测试成本翻倍——拖拽换列 / WIP 上限 / 未分类归类 / autoInit 每条都要走两遍
- 设计与零术语原则冲突——键位就是隐式术语，对 PM/设计师/运营不友好

**撤回后单测变化**：
- 之前：17 files / 223 tests
- 现在：16 files / 198 tests
- 撤回 1 个测试文件（`useKanbanKeyboardDrag.test.ts`）+ 14 个 `drag-helper.test.ts` 键盘用例 + 1 个 e2e 用例 = **共删 25 用例**，完全对得上 223 - 25 = 198。

**撤回后 e2e 变化**：
- `tests/e2e/board-drag.spec.ts` — 4 → 3 用例（用例 3 键盘场景删除）
- `tests/e2e/board-unassigned.spec.ts` — 4 用例（未受影响）
...
| `pnpm build` | 9.22s (含 toast.ts 的 dynamic+static 双导入 warning，与本次无关) ✅ |
| `pnpm test` | 16 files / 198 tests passed ✅ |
| `pnpm check:no-jargon` | OK ✅ |
| `pnpm vitest run tests/e2e/` | 2 files / 7 tests passed ✅ |
...
1. **真拖拽接入（vue-draggable-plus@^0.6.1）** — 替换旧的"按钮式换列"
   ...
   - **键盘双模已撤回**（v1.3.1）：Space 拾起 / 方向键切列 / Esc 取消 整体删除，聚焦鼠标唯一路径
...
|---|---|---|---|---|
| 1 | `635dc1c` | feat | 看板真拖拽接入 vue-draggable-plus + 键盘双模（**v1.3.1 撤回键盘双模**） | ✓ 1407→398 |
...
**4 件套**（v1.3 HEAD `3fc3ba0` 提交后；**注意**：键盘双模撤回后实际为 16 files / 198 tests，见 v1.3.1）：

| 检查 | 结果 |
|---|---|
| `pnpm type-check` | EXIT 0 ✅ |
| `pnpm build` | 6.45s (renderer) + 1.94s (main) + 27ms (preload) ✅ |
| `pnpm test` | 17 files / 223 tests passed（**v1.3.1 撤回后：16 files / 198 tests**）✅ |
| `pnpm check:no-jargon` | OK ✅ |
