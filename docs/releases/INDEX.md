...
- 当前最新 release note：[v0.8.32.md](./v0.8.32.md)（Windows GitHub 仓库刷新报错 `git: 'remote-https' is not a git command` 修复 — 入库 22 个 helper + DLL 到 `app/gitbinary/binaries/git/windows-helper/` + Init 释放 + 新增 `GitEnvFor` 注入 `GIT_EXEC_PATH` + Init 末尾 ls-remote smoke test + fetch 鉴权从 gh credential helper 改 env-based `GIT_CONFIG_*` 注入 `http.https://github.com.extraHeader=Authorization: Bearer <token>`；10 新测试；net +22 MB windows bundle）
- 上一版本：[v0.8.30.md](./v0.8.30.md)（fresh claim 单 commit segment 的双重 stitch 重复绘制 lane 修复 —— fork path 加自指跳过，避免 branch-off + 反向 fork stitch 叠加 stroke-width）
