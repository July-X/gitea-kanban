...
- 当前最新 release note：[v0.8.27.md](./v0.8.27.md)（Windows GitHub 仓库刷新报错 `remote-https is not a git command` 修复 — 内嵌 helper + DLL 21 个 + `GitEnvFor` 注入 `GIT_EXEC_PATH` + fetch 鉴权从 gh credential helper 改 env-based extraHeader + Init ls-remote smoke test；net +22 MB windows bundle；10 新测试）
- 上一版本：[v0.8.26.md](./v0.8.26.md)（Graph 算法入口统一收敛到 GitLens — 删 layout_vscode.go / layout.go / pickGraphBuilder env 开关 / 5 个 vscode 配套测试 / e2e baseline 对照块；净减 ~1700 行；DTO 共享类型迁到 types.go）
...
| **v0.8.27** | （待 push 后回填） | Windows GitHub 仓库刷新报错 `remote-https is not a git command` 修复。3 处改动：① `app/gitbinary/binaries/git/windows-helper/` 新增 21 个 helper + DLL（4 remote http exes + 16 libcurl/libssl/libcrypto/brotli/nghttp2/idn2/psl/ssh2/zstd 等）+ 子目录 `.gitignore` 白名单 ② `embed_windows.go` 嵌入 helper + `runner.go` Init 释放 + 新增 `GitEnvFor(binPath, extraEnv)` 注入 `GIT_EXEC_PATH`（仅内嵌路径触发）+ Windows 平台 Init 末尾 `ls-remote` smoke test 验证 ③ `app/git/native.go:fetchRemoteWithFilter` 鉴权改造：删 `-c credential.helper=!gh auth git-credential`，改 `GIT_CONFIG_COUNT/KEY_0/VALUE_0` env 注入 `http.https://github.com.extraHeader=Authorization: Bearer <token>`（限定到 github.com 域避免 token 泄露）+ `FetchWithFilter` 同步删 gh 路径检查 + 错误信息去重（删「输出: %s」重复拼接）。跨平台 stub：`helper_darwin.go` / `helper_other.go`。10 新测试：7 个 native_test.go（fetch args / GitEnvFor）+ 3 个 runner_e2e_test.go（Init 释放 + helper 链通 + bundle 端到端）| [v0.8.27.md](./v0.8.27.md) |
