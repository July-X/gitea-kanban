...
- 当前最新 release note：[v0.8.37.1.md](./v0.8.37.1.md)（ref badge 回退到「默认透明 + hover 填充」模式（v0.8.35.3 763e0ad 改的"默认填充"被 revert，对齐 vscode-gitlens `.gl-graph__ref-pill` 风格）。3 处改动：① `.ref-badge` 默认 `background-color: transparent` + `color: #E6EDF3` + `border: 1px solid var(--row-lane-color)` 浅字；② 恢复 `.ref-badge:hover` 填充变体（lane 色背景 + 文字变深）；③ badge 高度 22→20px + icon 20→18px（v0.8.37 改 `VSCODE_GRID_Y=24` 后底边 1px border 在 `col overflow:hidden` 下被裁，middle-turn 反馈「底边线丢失」修复）。pnpm typecheck 0 错）
- 上一版本：[v0.8.35.md](./v0.8.35.md)（ref badge 重做对齐 vscode-gitlens + 用户参考图——`.ref-badge` height 22→37（+68%），color `#E6EDF3` 白色 + 透明 bg + lane 色 border + hover 填充 lane 色 + 文字变深；`.ref-badge__icon` 18→30x30（+67%），margin-right 6→0（icon 紧贴 label 0 gap），stroke `currentColor`（hover 时跟 badge 文字色变），Tag/GitBranch SVG size 14→24（1.2rem，对齐 vscode-gitlens `--code-icon-size: 1.2rem`）；**`VSCODE_GRID_Y` 24→50**（让 37px badge 居中 row + 上下 padding ~13px）；9 处 test hardcode 改动态计算）
- 上一版本：[v0.8.34.md](./v0.8.34.md)（修复 Git Graph「已到全部提交记录的末尾」永远误显示——`app/git/graph/layout_gitlens.go:862` 硬编码 `Truncated: false` 导致 LogCommits 算的截断标志丢失，前端 maxCommits=300 截断后永远 allLoaded=true，root commit `327ac81` 不可见。`BuildGraphGitlens` 加 `truncated bool` 参数；2 个 adapter 传 `logResult.Truncated`；6 测试文件更新签名；新增 `TestBuildGraphGitlens_TruncatedPassesThrough` 回归。对齐 vscode-gitlens `hasMore` 语义——limit 触顶 → hasMore=true）
- 上一版本：[v0.8.33.1.md](./v0.8.33.1.md)（删除 `EnsureForkParentRemote`：只同步账号仓库基本信息——`app/platform/github/adapter.go:EnsureForkParentRemote` 在 `CloneRepo` + `PullRepoByProjectId` 自动触发，拉 fork parent（esengine/DeepSeek-Reasonix 等活跃开源项目）所有 `refs/heads/*` 含 23 个 dependabot PR branch，导致 50e77137 这类上游 bot commit 混进用户 fork 仓库的 git graph（maxLane 从 14 涨到 15）。改后 fork 仓库 graph 只显示用户 work-in-progress）
...
| **v0.8.33.1** | （待 push 后回填） | 删除 `EnsureForkParentRemote`：只同步账号仓库基本信息——用户 fork `esengine/DeepSeek-Reasonix` 后，应用数据目录仓库比源码仓库多 50 branches（23 个 `org/dependabot/*` PR branch + 5 个 `org/codex/*` + 5 个 `org/feature/*` + 多个 `org/fix/*`），应用 git graph 把上游 bot PR tip commit（如 50e77137）也画出来，maxLane 从 14 涨到 15。`app/platform/github/adapter.go:297 CloneRepo` + `app_sync.go:99 PullRepoByProjectId` 两处调用删除；`EnsureForkParentRemote` 函数体（拉 fork parent 全部 `refs/heads/*`）整体移除（与设计意图"拉 fork parent 装饰信息"脱节——实际拉了 23 个 dependabot PR branch 污染用户 fork 仓库 git graph）；`parentCloneURL` 保留（无副作用的纯网络 API 调用，供未来"显示 fork 关系"UI 用）。孤儿变量 `apiHostURL` 清理。新原则：只同步当前账号仓库基本信息，不关心 fork 源信息。注释（adapter.go:303-305）"显示 fork parent 发布 tag"设计意图被新原则取代——`parentCloneURL` 在前端 0 消费方（grep `frontend/src` 0 命中），实际行为与设计意图脱节，砍掉符合设计意图。 | [v0.8.33.1.md](./v0.8.33.1.md) |
| **v0.8.34** | （待 push 后回填） | 修复 Git Graph「已到全部提交记录的末尾」永远误显示。根因：`app/git/graph/layout_gitlens.go:862` 硬编码 `Truncated: false` 导致 `LogCommits` 算的 `truncated = (len(commits) > opts.MaxCount)` 截断标志在 graph 层丢失，前端 `TimelineNewView.vue:967 allLoaded = !dto?.truncated` 永远 true → 哨兵 v-if=`!allLoaded` 永远隐藏 → 用户无法触发 `loadMoreGraph`，root commit `327ac81` 永远不可见（`July-X/gitea-kanban` 1126 commits 应用只显示 301）。**修法**：`BuildGraphGitlens(commits, head, pinnedHeadShas, truncated bool)` 签名加 `truncated` 参数（替换硬编码 false）；`gitea/github adapter` `LogGraph` 调它时传 `logResult.Truncated`；`debug_dump_test.go` 同步用 `logResult.Truncated`（不再 hardcoded `false`）；6 个测试文件调用点更新签名。新增 `TestBuildGraphGitlens_TruncatedPassesThrough` 回归测试（2 子用例覆盖 true/false 透传）。**对齐 vscode-gitlens**：`packages/git-cli/src/providers/graph.ts` `hasMore = (limit && count >= limit)` 语义完全等价——limit 触顶 → hasMore=true → `renderLoadMoreRow`（vscode-gitlens 增量 cursor-based paging）/ `loadMoreGraph` sentinel 可见（我们 window-based paging）。验证：MaxCount=300 → truncated=true；MaxCount=2000 → truncated=false。12 个 Go 包测试全 PASS + frontend vitest 25 gitgraph 测试 PASS + pnpm typecheck OK。 | [v0.8.34.md](./v0.8.34.md) |
| **v0.8.35** | （待 push 后回填） | ref badge 重做对齐 vscode-gitlens + 用户参考图（vscode-gitlens `.gl-graph__ref-pill` 参考：font-size 1rem + padding 0.3rem 0.8rem + border 1px + `--code-icon-size 1.2rem` + 透明 bg + lane 色 border + hover 填充 lane 色 + 文字变 contrastColor）。**改 3 处**：① `.ref-badge` height 22→37（+68%），line-height 37px，padding `0 8px → 0 12px`，color `#E6EDF3` 白色（vscode-gitlens 对齐），background `transparent` + border `lane 色` + hover 填充 lane 色 + 文字变深（`--color-shell-main-bg`）；② `.ref-badge__icon` 18→30x30（+67%），margin-right 6→0（icon 紧贴 label 0 gap 对齐 gitlens ref-pill），stroke `--color-shell-main-bg` 写死→`currentColor`（hover 时跟 badge 文字色变）；Tag/GitBranch SVG size 14→24（1.2rem）；③ `frontend/src/lib/gitgraph/vscode-render.ts:VSCODE_GRID_Y` 24→50（让 37px badge 居中 row + 上下 padding ~13px，对齐用户参考图 22 的 50px row）。9 处 test hardcode path 数值（控制点 `12 + gridY * 0.8`、贝塞尔端点 `offsetY + gridY`、angular 拐点 `(offsetY + gridY) - gridY * 0.38`、fork stitch 转场段等）改 VSCODE_GRID_Y 动态计算 + `.toFixed(1)` 对齐 SVG path toFixed 精度（避免 24→50 后整数 hardcode 跟 SVG 输出的 `.0` 精度不匹配）。25 个 gitgraph 测试全 PASS + pnpm typecheck OK。 | [v0.8.35.md](./v0.8.35.md) |
| **v0.8.37.1** | （待 push 后回填） | ref badge 回退到「默认透明 + hover 填充」模式（v0.8.35.3 763e0ad 改的「默认 lane 色填充 + 深色文字」被 revert，对齐 vscode-gitlens `.gl-graph__ref-pill` 风格）。3 处改动：① `.ref-badge` 默认 `background-color: transparent` + `color: #E6EDF3` 浅字 + `border: 1px solid var(--row-lane-color)` lane 色边框（4 处 CSS 变量 + 1 行注释改动）；② 恢复 `.ref-badge:hover` 填充变体（`background-color: var(--row-lane-color)` + text 变深，对齐 gitlens）；③ badge 高度 22→20px + icon 20→18px（v0.8.37 改 `VSCODE_GRID_Y=24` 后底边 1px border 在 `col overflow:hidden` + sub-pixel 抗锯齿下被裁，mid-turn 反馈「底边线丢失」修复，box-sizing 显式声明 border-box 统一）。`pnpm typecheck` 0 错。关联 commits：763e0ad（v0.8.35.3 默认填充，被本 commit revert）、d256c7c（v0.8.35.4 icon 缩 10%，保留缩 10% 风格）。 | [v0.8.37.1.md](./v0.8.37.1.md) |
