...
## 6. 主题策略拍板（2026-06-10 12:12 用户决策）

**用户拍板**：v1 **单主题暗色**，**不提供切换**。

**同步更新**：
- `design-system/gitea-kanban/OVERRIDE.md` §"覆盖决策" 表 "背景" 行：从"默认浅色 + 暗色可切" → "v1 单主题暗色（不提供切换），背景 `#0d1117` 三层深色面板 + 8-12% 主色 alpha glow"
- `design-system/gitea-kanban/OVERRIDE.md` §"desktop 窗口专属" 第 4 条：从"暗色模式跟系统" → "v1 单主题暗色（不跟随系统）"
- `design-system/gitea-kanban/OVERRIDE.md` §"Skill 来源声明"：加 "v1 单主题暗色"
- `AGENTS.md` §6 "设计系统 token" 行：从"默认浅色 + 暗色可切" → "v1 单主题暗色（不提供切换）"
- `AGENTS.md` §8.2 "主题策略 v3→v4→v5 修正"：记录三次反复的教训——"M0 启动前任何主题策略变更必须经过用户拍板，worker 不准自由发挥"
- `docs/design/00-overview.md` §4 关键决策摘要 #8：从"零术语 + 二次确认 + 错误人话 + a11y 加强" → 加 "+ v1 单主题暗色（不提供切换）"

**为什么单主题暗色**（用户决策理由）：
1. 实施最快，避免分散精力（不维护两套 CSS 变量、不用做切换组件）
2. 暗色风符合 dev 工具桌面的常见范式（VSCode / Cursor / Linear）
3. 非技术用户友好通过"清晰文字 + 二次确认 + 零术语"保证，**不靠浅色**——03 §1 已经把这层写透

**已知不再需要决策的项**：
- ~~主题策略待拍板~~ ✅ 已拍板 = 单主题暗色
- ~~03 v3 文档里"默认暗色、不提供切换按钮"vs OVERRIDE "默认浅色 + 暗色可切"~~ ✅ 不再矛盾
