...
# AGENTS.md — gitea-kanban (v2.0 → v0.8.0)

> **本文件给所有 AI coding agent 和开发者读**。它是项目实现的入口规范；如果本文件与仓库里其它文档冲突，**以本文件为准**。
>
> 最后更新：2026-07-19（**v0.8.0 发版** — 应用自动更新（参考 DeepSeek-Reasonix 桌面端自更新架构）。后端新增 `app/updater` 包（manifest.go + updater.go + verify.go + 17 个 Go 单测），提供 Check/Download/Install/OpenDownloadPage 4 个核心方法 + ed25519 detached signature 校验 + SHA256 二次校验 + 3 次重试 + 断点续传 + 分平台 in-place apply（Windows 走 restart-helper.cmd 避开文件锁，macOS 未签名 build 走 OpenDownloadPage 手动下载页）；新增 7 个 Wails binding（Version/CheckUpdate/DownloadUpdate/InstallUpdate/OpenDownloadPage/SetCheckUpdatesPref/GetCheckUpdatesPref）+ app.go OnStartup 末尾 initUpdater + 异步 checkUpdatesAtStartup；前端新增 `useUpdate` composable（10 种 status 状态机）+ UpdateBanner 组件（5 种状态模板 + 进度条）+ 3 个 DTO（UpdateInfo/UpdateDownloadResult/UpdateProgress）；新增 `cmd/sign` CLI（ed25519 genkey/sign/manifest + argon2id 私钥加密）+ `.github/workflows/release.yml`（tag v* 触发跨平台 build + sign + publish）。Store 包新增 GetPrefBool/SetPrefBool helper + CheckUpdatesPrefKey 常量。目标平台：Windows + macOS 双平台（明文排除 Linux 和 canary 通道）。签名方案：Go 标准库 `crypto/ed25519`，零新依赖；公钥占位 `app/updater/verify.go` 的 `PublicKeyB64` 待 maintainer 替换为真实公钥（用 `go run ./cmd/sign genkey` 生成）。详细设计：`docs/releases/v0.8.0-plan.md`；发版记录：`docs/releases/v0.8.0.md`。
>
> **前版本**：**v0.7.19 发版** — label 方向判断改用 body 字段 + push event 渲染对齐 Gitea web：user 反馈"事件标记错误"（label 显示"移除了"实际是"添加了"）+ "UI 没有对齐"（push 事件 block 块多了短 SHA 前缀 + 多了 inline 块的 head commit 短码链接）。2 个根因：① v0.7.6 调研 Gitea 源码 `models/issues/comment.go: Content` 字段在 type=7 label change 时写 `"1"`=add，但 Gitea 1.26+ timeline 端点 label 事件**没有 `content` 字段**——label add/remove 信息在 `body` 字段（实测 pr72/pr81 timeline 数据 label event body 全是 `"1"`）。v0.7.6 写代码时把字段名搞错（`r.Content` 永远空串），label 判断永远走 remove 路径。修法：移除 v0.7.6 加的 r.Content struct 字段（json tag 改 `"body"` 会跟 Body 字段冲突，Go json.Unmarshal 同一 tag 多个字段会全部不填值），label 判断改用 `r.Body` 字段。② v0.7.8 push event 渲染多加了 inline 块 head commit 短码链接 + block 块短 SHA 前缀，跟 Gitea web 1:1 对齐冗余——Gitea web `commits_list_small` 模板只显示 commit 消息（带链接到 commit 页）+ author avatar，没有短 SHA 前缀。修法：删 push event inline 块 head commit 短码链接 + block 块 short SHA 链接改成 commit 消息链接 + force push 提示搬主行）
>
> **前版本**：**v0.7.18 发版** — TimelineItem DTO 字段名 camelCase + merge 事件真正搬主行：user 反馈"分支信息还是换行了" + "push、delete 事件的分支信息还是未显示"。2 个根因——① TimelineItem DTO 字段 snake_case（Go json tag）→ Wails 生成 snake_case TS class → 前端 dto.ts camelCase 强转后所有 camelCase 访问都拿 undefined（v0.7.6 / v0.7.8 / v0.7.9 加的功能基本都失效）；② v0.7.15 注释说要把 merge 事件 SHA + branch 拼到主行 verb 后但**实际没动代码**——span 还在 inline 块。修法：① Go json tag 改 camelCase + wails generate module 重新生成；② merge 事件 v-else-if 链 span 真正搬到主行。
...
| 维度 | 选型 | 说明 |
|---|---|---|
| 运行时 | **Go 1.26+** + Wails v2.12.0 | Go 编译为单一原生二进制；Wails 用系统 WebView |
