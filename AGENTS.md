...
- **双 ABI 循环陷阱**（vitest 跑 node 22 ABI=141，electron 41 要 ABI=145）：
  ...
  - 反之亦然
  - **解决**（package.json scripts 已修）：
    - `test` / `test:watch` / `test:coverage` 全部前置 `pnpm rebuild:native:node` 切回 node ABI
    - `dev` / `build` 前置 `pnpm rebuild:native:electron` 切回 electron ABI
    - 留 `dev:skip-rebuild` / `build:skip-rebuild`（无）给"明确知道 ABI 已对的"开发者用
