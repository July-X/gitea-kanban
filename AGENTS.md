...
12. **Windows 内嵌 git 不带 remote helper**（v0.8.27+ 修复）：内嵌 `gk-git-2.55.0-windows-amd64.exe` 是 MinGit 提取的 `cmd/git.exe` 单文件，没带 `git-remote-https.exe` 等 helper 也不带 libcurl/libssl DLL；`git fetch <https remote>` 会报 `remote-https is not a git command`。修法：把 helper + DLL 嵌入 `app/gitbinary/binaries/git/windows-helper/`、Init 释放到 `${dataDir}/tools/git/`、RunGitWithEnv 通过 `GitEnvFor(binPath, extraEnv)` 注入 `GIT_EXEC_PATH=<tools/git>`（仅内嵌路径触发；系统 git 不注入避免破坏相对 exec-path 解析）。fetch 鉴权从 `-c credential.helper=!gh auth git-credential` 改 env-based `GIT_CONFIG_*` 注入 `http.https://github.com.extraHeader=Authorization: Bearer <token>`（限定到 github.com 域避免 token 泄露给其它 remote；env 注入符合 §8.1 鉴权铁律）。详见 [docs/adr/0010](./docs/adr/0010-go-git-vs-git-cli.md) + [docs/releases/v0.8.27.md](./docs/releases/v0.8.27.md)。
