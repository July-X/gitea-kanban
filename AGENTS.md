...
>
> 最后更新：2026-06-22（**v2.0 重大迁移**：Electron+TypeScript+Vue → Go+Wails+Vue 3；单平台 Gitea → 多平台 Gitea+GitHub；前端保留 Vue 3，git 客户端改用 go-git；旧代码归档到 `legacy/electron/`。详见 [ADR-0005](./docs/adr/0005-electron-to-go-wails-migration.md)）
>
...
**Workspace 路径**（Git Graph 专用）：`prefs['app.workspacePath']`，默认 `~/.gitea-kanban/workspace`，repos 存 `${workspace}/repos/${owner}__${repo}/`。

### 6.5 git 客户端（go-git）

**`app/git/clone.go`**：`go-git.PlainClone` + `http.BasicAuth{Username, Password: token}`，token 仅内存态不落盘。
...
**`app/git/sync.go`**：`FetchRepo` + `PullRepo`（fetch + 统计 commit 变化 + SHA 变化检测）。
...
- Workspace：`${dataRoot}/workspace/repos/${owner}__${repo}/`（go-git clone 目标）
...
| 文档 | 路径 | 用途 |
|---|---|---|
| 设计综述 + 路线图 | `docs/design/00-overview.md` | 用户 review 入口（**v2 部分需更新**） |
...
| **v2.0 迁移决策** | `docs/adr/0005-electron-to-go-wails-migration.md` | **v2.0 重大决策（新增）**：Electron→Go+Wails + 多平台 + go-git |
