...
> 最后更新：2026-06-10（设计阶段收口；实现阶段开始后由后端/前端 agent 维护）
...
|---|---|
| **框架** | **React 18 + Vite** |
| **状态管理** | **Zustand**（不用 Redux / Pinia / Context） |
| **路由** | **React Router 6** |
| **UI 组件库** | **Radix UI Primitives + 自定义样式**（不引 antd，视觉太重） |
| **样式** | **CSS Modules + 全局 CSS 变量**（不引 Tailwind / styled-components） |
| **timeline / git graph** | **AntV X6@3.1.7**（图编辑引擎，git graph 范式最契合） |
...
│   │   ├── stores/                 # Zustand 切片（auth / repo / board / branch / pr / timeline / sync / ui / settings）
...
| **渲染进程单测** | Vitest + RTL | ≥ 60% 行覆盖 | 组件渲染 / Zustand store action / 危险操作二次确认触发 |
...
- **TS / TSX 格式化**：`prettier`（统一配置在 `.prettierrc`）—— `prettier --write "src/**/*.{ts,tsx}"`
- **Lint**：`eslint` + `@typescript-eslint`（统一配置在 `.eslintrc.cjs`） —— `pnpm lint`
...
- 所有用户输入走 React 转义，**禁止** `dangerouslySetInnerHTML`
...
| **文件 boundary** | **不**碰 `src/renderer/**`、**不**写 React 组件、**不**写 CSS、**不**直接调 gitea API 而绕过 IPC |
...
| **依赖** | `electron`, `better-sqlite3`, `drizzle-orm`, `pino`, `keytar`, `openapi-fetch` |
...
| **依赖** | `react`, `react-dom`, `react-router-dom`, `zustand`, `@antv/x6`, `zod`, `@radix-ui/*`, `lucide-react` |
...
1. **改技术栈**：Electron / TS / React / X6 / SQLite / Drizzle / Zustand 任一变更
...
> gitea-kanban = **Electron 桌面应用 + React/X6 渲染进程 + Node 主进程 + SQLite + gitea PAT**。
