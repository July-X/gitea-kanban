...
# AGENTS.md — gitea-kanban (v2.0)

> **本文件给所有 AI coding agent 和开发者读**。它是项目实现的入口规范；如果本文件与仓库里其它文档冲突，**以本文件为准**。
>
> 最后更新：2026-06-22（**v2.0 重大迁移** + **v2.4 迭代收尾** + **v2.5 workspace 按账号分层**）
>
> - **v2.0**：Electron+TypeScript+Vue → Go+Wails+Vue 3；单平台 Gitea → 多平台 Gitea+GitHub；前端保留 Vue 3，git 客户端改用 go-git；旧代码归档到 `legacy/electron/`。详见 [ADR-0005](./docs/adr/0005-electron-to-go-wails-migration.md)
> - **v2.4**：迁移完成后真实用户桌面跑暴露 6 类问题（鉴权铁律 / 业务 binding stub / 数据目录嵌套 / StatusBar localPath 拼错 / prefs 死链 / go-git 拉全 worktree），全部修复并记录在 [ADR-0006](./docs/adr/0006-v24-iteration-fixes.md) + [07-v24-iteration.md](./docs/design/07-v24-iteration.md)。关键：所有 binding 接受 `projectId` / `owner+repo` 业务态概念，Go 端反查 `localPath + token`；go-git 走 `NoCheckout=true` 轻量模式（磁盘 -99%）；prefs 走 IPC + localStorage 双源持久化
> - **v2.5**：workspace 按账号分层。`${dataDir}/workspace/repos/${owner}__${repo}/` → `${dataDir}/workspace/repos/${username}/${owner}__${repo}/`。多账号场景避免同名 username 在不同平台撞目录；启动期**自动迁移**旧数据，备份保留到 `${dataDir}/workspace/_pre_v25_workspace`。详见 §6.4 + §6.5
>
...
### [x] uncommitted changes 灰色虚线（v3.x）

**实现状态：** Go 后端 + vscode 风格 / Gitea 风格前端都接通，9 个新测试全过。

**触发语义：** 复刻 vscode `commits[0].hash === UNCOMMITTED` 模式，但 vscode 靠
`git status --porcelain` 探测 worktree 变更，**我们 NoCheckout 模式没 worktree，
那条路走不通**。改为「local HEAD 落后于 origin/<defaultBranch>」——

- `app/git/log_vscode.go` 的 `detectUnpulledCommits(localPath)` 用 git CLI
  探测 `localHead..origin/<defaultBranch>` ahead 数（探测顺序
  `origin/HEAD > origin/main > origin/master`）
- 探测到则 prepend `UNCOMMITTED_HASH`（' *' 哨兵）虚拟 commit 到
  **local HEAD 之前**（不是直接 unshift 到最前 —— all-branches 视图下
  commits[0] 可能是 origin/main 上的 commit；按 vscode dataSource.ts:186-196
  的 insert 位置逻辑）
- 字段对齐 vscode `dataSource.ts:191`：parents=[headSHA], author='*', email='',
  subject="N commits from origin ahead"

**渲染层：**
- `app/git/graph/layout_vscode.go` 加 `vsVertex.setNotCommitted()` +
  `vsLine.isCommitted` 字段（对齐 graph.ts:287-289 + 102）
- `app/git/graph/layout.go` 检测 `c.SHA == UNCOMMITTED_HASH` 写
  `IsCommitted: false`（Gitea 风格）
- `GraphBranchLine.IsCommitted` 字段透传到前端 DTO
- 前端 `vscode-render.ts` 的 `addPath` 接 isCommitted，isCommitted=false
  时走 `#808080` + `stroke-dasharray: 2px`（对齐 Branch.drawPath graph.ts:152-155）
- branch 绘制按 line.isCommitted 切路径（对齐 Branch.draw graph.ts:125）
- `structured.ts` (Gitea 风格) 同样支持 isCommitted/dasharray 透传
- `TimelineNewView.svgCircleNodes` 给 `sha === '*'` 强制 `isCurrent=true`
  让 UNCOMMITTED 顶点画成空心圆（对齐 OpenCircleAtTheUncommittedChanges 风格）

**端到端验证：** clone → fetch 后未 pull 时，graph 顶部应出现空心灰圆 +
"N commits from origin ahead" 行 + 一段灰色虚线从空心圆到本地 HEAD 圆点。

**涉及文件：**
- `app/git/log_vscode.go`：UNCOMMITTED_HASH + detectUnpulledCommits + buildUncommittedCommit + LogCommitsVscode 插入
- `app/git/log.go`：go-git 路径 LogCommits 同样接 detect + 插入
- `app/git/graph/layout_vscode.go`：setNotCommitted + vsLine.isCommitted + loadCommits 检测
- `app/git/graph/layout.go`：GraphBranchLine.IsCommitted 字段 + Gitea 风格 IsCommitted=false
- `frontend/src/lib/gitgraph/structured.ts`：GraphBranchLineDto.isCommitted + SvgPath.isCommitted/dasharray + addPath 切色
- `frontend/src/lib/gitgraph/vscode-render.ts`：addPath 接 isCommitted + branch 切路径
- `frontend/src/views/TimelineNewView.vue`：PathGroup 透传 + svgCircleNodes 给 UNCOMMITTED 强制 isCurrent=true + 模板 path 绑 stroke-dasharray
- 测试：`app/git/log_uncommitted_test.go` (6 个) + `app/git/graph/layout_uncommitted_test.go` (3 个)
