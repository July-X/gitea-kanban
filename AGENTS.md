...
### [ ] dot hover tooltip 内容补全（对齐 vscode-git-graph graph.ts showTooltip）

**VSCode tooltip 完整结构：**
- Title: `Commit <shortHash>`
- Section: `This commit is <not> included in HEAD`（childrenIncludesHead 判断）
- Section: `Branches: <branchName> <remote>`（带 HTML 高亮 ref）
- Section: `Tags: <tagName>`（带 HTML 高亮）
- Section: `Stashes: <stashName>`（带 HTML 高亮）
- Section: `<ELLIPSIS>`（超过 10 个 ref 时折叠）
- Pointer/anchor: 用 `color = var(--git-graph-colorN)` CSS var
- Border: `border-color: color`
- Max width: `min(contentWidth - x - 35, 600)px`

**参考源码：** `~/2026/code/vscode-git-graph/web/graph.ts:811-894` (showTooltip)

**前端文件：** `frontend/src/views/TimelineNewView.vue`（dotEnter / tooltipContent / tooltip template）

### [ ] uncommitted changes 灰色虚线

**问题：** `NoCheckout: true` 模式下工作区永远为空，Go 后端 `IsCommitted` 始终为 `true`。

**VSCode 实现：** `commits[0].hash === UNCOMMITTED` 时创建一个虚拟顶点，stroke 颜色为 `#808080`。

**涉及文件：**
- `app/git/graph/layout_vscode.go`：Vertex.setNotCommitted + `numUncommitted` 计数
- `app/git/log.go`：`LogGraph` 返回时检测 worktree 是否有变更
- `frontend/src/lib/gitgraph/vscode-render.ts`：lane 0 底部画灰色虚线 path
- `frontend/src/views/TimelineNewView.vue`：tooltip 定位（已修复 pointer-events）
