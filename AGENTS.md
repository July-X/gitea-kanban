...
>
> 最后更新：2026-07-20（**v0.7.20 发版** — Mac 安装版 gh CLI 未安装时错误引导：新增 `gh_not_installed` 错误码 + `ipc.NewGhNotInstalled` 构造函数；`native.go` 的 `CloneWithFilter` / `FetchWithFilter` gh 缺失检查改为返回结构化 IpcError；前端 `buildGhInstallToastError` helper 生成带"打开安装页"按钮的 toast（persistent + actions）；StatusBar / TimelineNewView 的 sync catch 块加 `if e.code === 'gh_not_installed'` 分支；2 个 Go 回归测试（clone_test.go / sync_test.go）。详细发版记录：`docs/releases/v0.7.20.md`。
>
