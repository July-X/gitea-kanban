...
- **日志位置**：`$GITEA_KANBAN_DATA_DIR/logs/main/main-YYYY-MM-DD.log` 或 `~/.gitea-kanban/logs/main/main-YYYY-MM-DD.log`（跟 db 同根，详见 §8.16），按日滚动，保留 14 天。
...
- **跟 §4.7 安全边界的关系**：
  ...
  - 日志路径原本走 `app.getPath('logs')`（macOS = `~/Library/Logs/gitea-kanban/main/`）—— 2026-06-11 §8.16 拍板改为**跟 db 同根走 `~/.gitea-kanban/logs/`**，理由详见 §8.16
...
### 8.16 日志路径跟 db 同根走家目录 `~/.gitea-kanban/logs`（2026-06-11）

- **背景**：§8.15 把 db 路径统一到 `~/.gitea-kanban/` 后，**日志还留在 `app.getPath('logs')`**（macOS = `~/Library/Logs/gitea-kanban/main/`、Windows = `%APPDATA%/gitea-kanban/logs/`）——这导致：
  - db 跟日志**分家**，多实例场景下用户备份/迁移要管两个目录
  - 跨平台日志路径语义仍不一致（macOS `Library/Logs/` / Windows `%APPDATA%/` / Linux `~/.config/`）
  - 调试时定位问题要跨目录（db 在家目录、log 在 Library —— 这次 §8.11 那次 owner-skip 翻车根因之一）
- **决策**（user 拍板 2026-06-11）：日志**跟 db 同根**，走 `$GITEA_KANBAN_DATA_DIR/logs/` 或 `~/.gitea-kanban/logs/`。
- **改动**：
  - `src/main/logger.ts:resolveLogDir()` 改为 `<dataRoot>/logs/<LOG_SUBDIR>` —— 复用 §8.15 的 `GITEA_KANBAN_DATA_DIR` env 变量 + home fallback
  - `src/shared/constants.ts:LOG_SUBDIR` 注释同步
  - 删 `app.getPath('logs')` 调用（**注意** logger.ts 仍 `import { app }` 因为其他用 `app.isPackaged()`，只删 `getPath`）
- **跟 §8.15 的关系**：
  - 两者**共用同一个根** + 同一个 env 变量（`GITEA_KANBAN_DATA_DIR`）——多实例时 db + log 在一起
  - **不**新增独立 env 变量 `GITEA_KANBAN_LOG_DIR`（避免 env 树分散；如未来需要可加 §8.17）
  - **保留** 14 天保留期 + 启动时 `cleanupOldLogs` 清理机制
- **影响范围**（已同步）：
  - `src/main/logger.ts:resolveLogDir()` + `resolveDataRoot()` — 已重写
  - `src/shared/constants.ts:LOG_SUBDIR` 注释 — sync
  - **待补**：`docs/design/02-architecture.md` §4（数据存储）/ §9（日志滚动）/ §9（跨设备迁移）— sync 待办
- **跨平台路径**：
  | 平台 | 路径 |
  |---|---|
  | macOS / Linux | `~/.gitea-kanban/logs/main/main-YYYY-MM-DD.log` |
  | Windows | `%USERPROFILE%\.gitea-kanban\logs\main\main-YYYY-MM-DD.log` |
- **verifier 验证项**（同步到 §5.3）：
  - 单测断言 `resolveLogDir()` 默认返回 `path.join(os.homedir(), '.gitea-kanban/logs/main')`
  - 临时设 `GITEA_KANBAN_DATA_DIR=/tmp/foo` 后断言返回 `/tmp/foo/logs/main`
  - `cleanupOldLogs` 仍按 14 天 mtime 清理
- **踩坑教训**（给未来 worker）：
  - **不要再走回 `app.getPath('logs')`**——理由同 §8.15（dev 模式 + productName 缺失时路径错位）
  - **多实例时**（v2 多账号场景）所有 instance 共享 `~/.gitea-kanban/logs/` 还是各自走 `GITEA_KANBAN_DATA_DIR`？当前**默认共享**（v1 不做多实例），v2 评估
  - **Windows 域控 `os.homedir()` roaming profile** 同 §8.15 风险——v1 接受 fallback，v2 评估
- **跟 §4.7 安全边界的关系**：日志路径**不**接受用户输入（只读 `GITEA_KANBAN_DATA_DIR` env 变量 + home fallback），env 校验 `isAbsolute` —— 跟 db 同源、跟 §4.7 不冲突
