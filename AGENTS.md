...
14. **vue-draggable-plus 0.6.1 不传 modelValue → options 全丢（2026-06-18 踩坑 · 看板拖拽失效真因）**：`VueDraggable` 组件的 `useDraggable(target, modelValue, options)` 有参数解析坑 —— `Array.isArray(unref(modelValue)) || (options = modelValue, modelValue = null)`。当**不传** `:model-value`（旧 KanbanColumnSection 只 `v-bind="dragOptions"`）时，modelValue 的 unref 是 undefined（非数组）→ 把本应是 options 的第 2 参误当 modelValue → **真正的 options（group/animation/onStart/onMove/onEnd/ghostClass 等）全部丢弃** → SortableJS 用**默认 options** 创建 → 没有 onStart/onMove/onEnd 回调 → `@start/@move/@end` emit 不触发 → 拖拽光晕失效 + 释放不记录（moveIssue 不调）。**修法**：`<VueDraggable :model-value="displayIssues" v-bind="dragOptions" ...>`，传真实数组让参数解析走对分支；**不**监听 `@update:model-value`（store 仍是 source of truth，Sortable 改 DOM 的视觉效果会被 Vue 下一帧重渲染拉回，与 drag-helper.ts 注释一致）。**所有用 VueDraggable 的地方**（KanbanColumnSection / UnassignedSection）都必须传 `:model-value`。**守护测试**：`KanbanColumnSection-modelvalue.test.ts` 直接检查 ul 上 SortableJS 实例的 `options.group/onStart/onMove/onEnd`。
15. **happy-dom 单测抓不到 vue-draggable-plus 的 bug（2026-06-18 踩坑）**：`useKanbanMouseDrag-glow.test.ts` 直接调 `onColumnDragStart`（绕过 VueDraggable），所以单测全过但真实拖拽坏。**教训**：composable 的纯逻辑单测 ≠ 集成链路验证。涉及第三方库（SortableJS/vue-draggable-plus）的事件/选项透传时，必须在**真实 Chromium + Electron renderer** 用 CDP 验证 SortableJS 实例的 options。**CDP 验证方式**（参考 §8.7.7 模板）：(a) 拿 ul 上 SortableJS 实例（字符串 key `Sortable` + timestamp，**不是** symbol）；(b) 直接调 `sortable.options.onStart/onMove/onEnd(fakeEvt)` 验证 emit 链路；(c) **注意**：`Input.dispatchMouseEvent` 模拟拖拽有时序陷阱 —— SortableJS 用 `setTimeout(0)` 调度 `_dragStarted`，CDP 合成鼠标事件太快会让 `_dragStartId` 被 `_onDrop` 的 `clearTimeout` 取消 → onStart 不触发（这是 CDP 模拟的假象，不是真实 bug）。**可靠验证**用 direct 调 `options.onStart/onMove/onEnd`，别用 `Input.dispatchMouseEvent` 验拖拽事件链路。
