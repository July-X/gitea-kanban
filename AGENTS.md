...
>
> 最后更新：2026-06-22（**v2.0 重大迁移** + **v2.4 迭代收尾**）
>
...
**Workspace 路径**（Git Graph 专用，**v2.2 锁定不可改**）：`~/.gitea-kanban/workspace`（单层，无嵌套），repos 存 `${workspace}/repos/${owner}__${repo}/`。**禁止**在连接 / 设置界面暴露修改入口（v2.2 user 拍板）；设置界面只读 + "打开应用数据目录"按钮（`App.OpenDataDir`）。详见 [ADR-0006 §2.3](./docs/adr/0006-v24-iteration-fixes.md#23-数据目录收尾-v22--v24)。
