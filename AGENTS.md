...
> **⚠️ 版本演进信息不在本文件维护**：所有版本的演进历史 / release note 索引统一在 [docs/releases/INDEX.md](./docs/releases/INDEX.md) 维护。本文件**仅承载与版本无关的规范章节**（项目概述 / 技术栈 / 架构 / 安全 / 测试 / ADR 索引等），保持稳定。
...
11. **git / gh / go-git 工具分工**（v0.7.22 用户拍板）：写操作（clone/fetch）按平台分（GitHub 优先 `gh`，Gitea 走 `git` CLI），读 DAG 统一用 `go-git`——go-git 的 PlainClone 不支持 `--filter=blob:none`，大仓库会下 28GB blob，必须切到 git CLI。详细规范 + 迁移计划见 [`docs/adr/0010-go-git-vs-git-cli.md`](./docs/adr/0010-go-git-vs-git-cli.md)。
12. **Windows 内嵌 git 不带 remote helper**（v0.8.32+ 修复）：内嵌 `gk-git-2.55.0-windows-amd64.exe` 是 MinGit 提取的 `cmd/git.exe` 单文件，没带 `git-remote-https.exe` 等 helper 也不带 libcurl/libssl DLL；`git fetch <https remote>` 会报 `git: 'remote-https' is not a git command`。修法：把 helper + DLL 嵌入 `app/gitbinary/binaries/git/windows-helper/`、Init 释放到 `${dataDir}/tools/git/`、RunGitWithEnv 通过 `GitEnvFor(binPath, extraEnv)` 注入 `GIT_EXEC_PATH=<tools/git>`（仅内嵌路径触发；系统 git 不注入避免破坏相对 exec-path 解析）。fetch 鉴权从 `-c credential.helper=!gh auth git-credential` 改 env-based `GIT_CONFIG_*` 注入 `http.https://github.com.extraHeader=Authorization: Basic base64(x-access-token:<pat>)`（v0.8.32.1 修订：v0.8.32 误用 GitHub REST API 的 `Bearer` 头到 Git Smart HTTP 协议 → 私有仓库 fetch 永远 401；正确格式是 Smart HTTP 要求的 `Basic(x-access-token:PAT)`，username 固定为 `x-access-token`，password 是 PAT 原文，base64 是传输编码不是脱敏）。URL 限定到 `http.https://github.com.extraHeader`（不是全局 `http.extraHeader`）避免 token 泄露给其它 gitea remote host；env 注入符合 §8.1 鉴权铁律。401 / `remote: invalid credentials` 关键字识别 → 返回 `ipc.NewGitHubAuthFailed` 引导重连。详见 [docs/adr/0010](./docs/adr/0010-go-git-vs-git-cli.md) + [docs/releases/v0.8.32.md](./docs/releases/v0.8.32.md) + [docs/releases/v0.8.32.1.md](./docs/releases/v0.8.32.1.md)。
...
| **GitGraph 引擎演进历程** | `docs/design/gitgraph-engine-history.md` | **v0.8.25 新增**：四阶段演进（字符流→layout.go→vscode 1:1→GitLens 移植）+ 5 个具体 bug 修复（cherry-pick 排序 / uncommitted lane 泄漏 / 单 commit 不画线 / DTO 转换 4 bug / 连线形态 4 错）+ 配色优化过程。**v0.8.33 追加**「阶段 11」：回退 v0.8.25.7 「lane 左移压缩」对齐 GitLens 原版算法 + 4 条踩坑教训（不要凭印象实现算法 / 小仓库验证不能外推到大仓库 / 肉眼对比不能量化 / 不要为"消除双跑"以外的任何目的引入双跑） |
