...
import type { ColumnDto, IssueCardDto, IssueLabelDto } from '@renderer/types/dto';
import { useGlobalLoadingStore } from '@renderer/stores/global-loading';
import { clusterLabels, type ClusterPlan, type ColumnGroup } from '@renderer/lib/label-cluster';
import { matchIssueToColumn } from '@renderer/lib/issue-column-match';

const CANONICAL_COLUMN_TITLES = ['新建', '待办', '进行中', '已完成'] as const;
const CANONICAL_COLUMN_LABEL_ALIASES: Record<(typeof CANONICAL_COLUMN_TITLES)[number], string[]> = {
  新建: ['新建', 'Backlog'],
  待办: ['待办', '待处理', 'To Do'],
  进行中: ['进行中', '处理中', 'In Progress'],
  已完成: ['已完成', 'Done'],
};
...
export const useBoardStore = defineStore('board', () => {
  ...
  async function loadBoard(projectId: string): Promise<LoadBoardResult> {
    ...
    try {
      ...
      // === 自动初始化：0 列 → 创建 4 个基础列；有 gitea label 时顺手按聚类绑定 ===
      let autoInitBreakdown: ClusterPlan | null = null;
      if (cols.length === 0) {
        // 跑 cluster（纯函数），拿 breakdown 返给 caller 弹 toast；无 label 时保持 null，避免展示空 breakdown。
        const plan = clusterLabels(labelsResp.items);
        autoInitBreakdown = labelsResp.items.length > 0 ? plan : null;
      ...
      return {
        columns: resultColumns,
        autoInitCreatedCount: resultAutoInitCount,
        autoInitBreakdown,
      };
    } catch (e) {
      error.value = normalizeError(e);
  ...
  async function autoInitColumns(
    ...
    const groupsByTitle = new Map<string, ColumnGroup>();
    for (const group of [...plan.literal, ...plan.prefixGroup, ...plan.compound]) {
      groupsByTitle.set(group.columnTitle, group);
    }
    // v1.4 调整（2026-06-20）：首次进入固定生成 4 个基础列，保证看板稳定是
    // 「新建 / 待办 / 进行中 / 已完成」四列；匹配到的 gitea label 自动绑定，没匹配的列留给用户手动绑定。
    const canonicalGroups: ColumnGroup[] = CANONICAL_COLUMN_TITLES.map((title) => {
      const labelIds = CANONICAL_COLUMN_LABEL_ALIASES[title].flatMap(
        (alias) => groupsByTitle.get(alias)?.labelIds ?? [],
      );
      return { columnTitle: title, labelIds };
    });
    const canonicalAliasSet = new Set(Object.values(CANONICAL_COLUMN_LABEL_ALIASES).flat());
    const extraGroups = [...plan.literal, ...plan.prefixGroup, ...plan.compound].filter(
      (group) => !canonicalAliasSet.has(group.columnTitle),
    );
    const allGroups = [...canonicalGroups, ...extraGroups];
  ...
  async function updateIssueLabels(args: {
    ...
    const locateIssue = (): {
      issue: IssueCardDto;
      ref: { kind: 'column'; id: string } | { kind: 'unassigned' } | { kind: 'closed' };
    } | null => {
      if (colId) {
    ...
    const writeBack = (iss: IssueCardDto): void => {
      const nextByColumn: Record<string, IssueCardDto[]> = {};
      for (const [colId, list] of Object.entries(issuesByColumn.value)) {
        nextByColumn[colId] = list.filter((i) => i.index !== args.issueIndex);
      }
      let nextUnassigned = unassignedIssues.value.filter((i) => i.index !== args.issueIndex);
      let nextClosed = closedIssues.value.filter((i) => i.index !== args.issueIndex);

      if (iss.state === 'closed') {
        nextClosed = [...nextClosed, iss].sort((a, b) => a.index - b.index);
      } else {
        const nextColId = columnIdFromIssueLabels(iss);
        if (nextColId) {
          nextByColumn[nextColId] = [...(nextByColumn[nextColId] ?? []), iss].sort(
            (a, b) => a.index - b.index,
          );
        } else {
          nextUnassigned = [...nextUnassigned, iss].sort((a, b) => a.index - b.index);
        }
      }

      issuesByColumn.value = nextByColumn;
      unassignedIssues.value = nextUnassigned;
      closedIssues.value = nextClosed;
    };
  ...
  async function assignUnassignedIssue(args: {
    ...
    unassignedIssues.value = unassignedIssues.value.filter((i) => i.index !== args.issueIndex);
  ...
  async function createColumn(args: { projectId: string; title: string }): Promise<ColumnDto> {
    ...
    try {
      ...
      // 追加到 columns 列表末尾；autoInit 连续创建时保持「新建 / 待办 / 进行中 / 已完成」顺序。
  ...
  async function unmapLabelFromColumn(args: {
    columnId: string;
    giteaLabelId: number;
  }): Promise<void> {
    error.value = null;
