 ...
 *
 * ============================================================
 * 🚧 TODO(v0.6+): 把海豚位置从「主区居中」改成「App 右侧功能区」
 * ============================================================
 * 用户原始需求（pre-v0.4.0）：
 *   「调整 App 的加载动画，做一个全局的加载动画，
 *     出现的位置主要是 App 右侧的功能区。
 *     动画效果为：一个小海豚转圈。」
 *
 * 当前状态（v0.4.0 拍板 2026-07-02 by user）：
 *   - 位置维持「主区居中」（绝对定位 + flex 居中，line 90-102）
 *   - 动画保留 v1.4 海豚 emoji + 公转 + 头顶小气泡（line 144 + line 167）
 *     —— 公转本质就是「转圈」，与用户描述一致
 *   - 本次不动 position；改动抽到 v0.5 或 v0.6 单独 plan：
 *     1. .dolphin-overlay position:absolute → 仍 absolute；top: 50%; right: var(--space-3)
 *     2. 同步改写 z-index（避免遮挡 NavRail 当前右侧功能区）
 *     3. AppShell.vue 主区相对定位父级不需要改
 *     4. 视觉回归 + e2e（W3 e2e 已知 sign-in 流程的 loading 截图对照）
 *
 * 标签 `🚧 TODO(v0.6+)` 必须保留显眼，让下一个 agent 不漏这事。
 * ============================================================
 *
