...
test('GitHub fallback 实际渲染：多 PR 用父子边画线，dot 落在对应边端点上', () => {
  ...
  assert.ok(paths.length <= graph.flows.size, '实际 SVG path 按 flow 聚合，不再按父子边拆碎');
...
test('DeepSeek-Reasonix 顶部回归：第 5 条 commit 连到第 8 条 parent commit', () => {
  ...
  assert.ok(
    layout.paths.some(
      (p) =>
        pathTouchesPoint(p.d, layoutDotPoint(from, displayRowMap, layout).x, layoutDotPoint(from, displayRowMap, layout).y) &&
        pathTouchesPoint(p.d, layoutDotPoint(to, displayRowMap, layout).x, layoutDotPoint(to, displayRowMap, layout).y),
    ),
    '第 5 条 commit 和第 8 条 parent commit 必须附着在同一条 flow path 上',
  );
});
