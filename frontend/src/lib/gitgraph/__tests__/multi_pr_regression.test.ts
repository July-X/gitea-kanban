...
test('GitHub 多列字符流：边缘分支 path 从主线接出并回到主线，右侧仍只显示 commit 行', () => {
  ...
  assert.ok(d.includes(`M ${laneX(1)} 60 L ${laneX(1)} 90`), `侧线 commit 之间保持垂直短段，实际: ${d}`);
  assert.ok(d.endsWith(`L ${laneX(0)} 210`), `分支线按 / glyph 回到下一条主线 commit 行，实际: ${d}`);
});
...
test('GitHub 连续多 PR 字符流：每条侧线都保持同样接入/回合表现', () => {
  ...
  assert.ok(paths.some((d) => d.endsWith(`L ${laneX(0)} 90`)), `PR1 按 / glyph 回到 main row: ${paths.join(' | ')}`);
  assert.ok(paths.some((d) => d.endsWith(`L ${laneX(0)} 180`)), `PR2 按 / glyph 回到 main row: ${paths.join(' | ')}`);
});
...
test('GitHub fallback 实际渲染：多 PR 用父子边画线，dot 落在对应边端点上', () => {
  ...
  assert.equal(paths.length, visibleEdgeCount, '实际 SVG path 数量等于可见父子边数量，不再等于 glyph 段数量');
