...
export namespace platform {
	...
	export class TimelineItem {
	    ...
	    refAction?: string;
	    refCommitSha?: string;
	    dependentIssue?: IssueDTO;
	    isWipToggle?: boolean;
	    isWip?: boolean;
	    addedLabels?: PullLabelDTO[];
	    removedLabels?: PullLabelDTO[];
	    labelAction?: string;
	    commitIds?: string[];
	    isForcePush?: boolean;
	    mergeCommitSha?: string;
	    ...
	    constructor(source: any = {}) {
	        ...
	        this.commitSha = source["commitSha"];
	        this.oldTitle = source["oldTitle"];
	        this.newTitle = source["newTitle"];
	        this.oldRef = source["oldRef"];
	        this.newRef = source["newRef"];
	        this.label = this.convertValues(source["label"], PullLabelDTO);
	        this.oldMilestone = this.convertValues(source["oldMilestone"], MilestoneDTO);
	        this.milestone = this.convertValues(source["milestone"], MilestoneDTO);
	        this.assignee = this.convertValues(source["assignee"], PullUserDTO);
	        this.removedAssignee = source["removedAssignee"];
	        this.refIssue = this.convertValues(source["refIssue"], IssueDTO);
	        this.refAction = source["refAction"];
	        this.refCommitSha = source["refCommitSha"];
	        this.dependentIssue = this.convertValues(source["dependentIssue"], IssueDTO);
	        this.isWipToggle = source["isWipToggle"];
	        this.isWip = source["isWip"];
	        this.addedLabels = this.convertValues(source["addedLabels"], PullLabelDTO);
	        this.removedLabels = this.convertValues(source["removedLabels"], PullLabelDTO);
	        this.labelAction = source["labelAction"];
	        this.commitIds = source["commitIds"];
	        this.isForcePush = source["isForcePush"];
	        this.mergeCommitSha = source["mergeCommitSha"];
	    }
